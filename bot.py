...
# ⚠️  REPLACE THIS with a fresh token from github.com/settings/tokens (scope: repo)
# Your old token was exposed in public code and is likely revoked by GitHub.
GITHUB_TOKEN  = "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"
...
# ══════════════════════════════════════════════════════
#  MAINTENANCE MODE
#  Set MAINTENANCE = True to block all non-owner commands
# ══════════════════════════════════════════════════════
MAINTENANCE = False
...
async def maintenance_check(update: Update) -> bool:
    """Returns True if bot is in maintenance and user is NOT the owner."""
    if not MAINTENANCE:
        return False
    uid = update.effective_user.id if update.effective_user else None
    if uid and int(uid) == OWNER_ID:
        return False  # Owner bypasses maintenance
    if update.message:
...
async def gh_push(repo_path: str, local_path: str) -> bool:
    if not GITHUB_TOKEN or GITHUB_TOKEN == "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" or not GITHUB_REPO:
        logger.warning("GH push skipped — GITHUB_TOKEN not set.")
        return False
    repo = _gh_repo()
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if resp.status_code == 200:
                ...
                logger.error("GH push FAILED — token is invalid or expired! Generate a new one.")
                return False
            elif resp.status_code == 403:
                logger.error("GH push FAILED — token lacks 'repo' permission.")
                return False
            payload: dict = {
...
async def gh_pull(repo_path: str, local_path: str) -> bool:
    if not GITHUB_TOKEN or GITHUB_TOKEN == "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" or not GITHUB_REPO:
        logger.warning("GH pull skipped — GITHUB_TOKEN not set.")
        return False
    repo = _gh_repo()
    try:
        url = f"{GH_BASE}/repos/{repo}/contents/{repo_path}"
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if resp.status_code == 401:
                logger.error("GH pull FAILED — token is invalid or expired! Generate a new one.")
                return False
            if resp.status_code == 200:
...
async def save_and_push(d: dict):
    save_local(d)
    ok = await gh_push("data.json", DATA_FILE)
    if not ok:
        logger.warning("save_and_push: GitHub push failed — data saved locally only.")
...
async def broadcast(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    # NOTE: broadcast is NOT blocked by maintenance — admin still needs to reach users
    d   = load()
    uid = update.effective_user.id
    if not is_admin(uid, d):
        await update.message.reply_text("Admins only.")
...
async def syncgithub(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ok1 and ok2:
        ...
        result = "⚠️ Sync failed — GitHub token may be invalid or expired. Check logs."
    else:
        result = "⚠️ Sync done with some errors on DB files. Check logs."
    log_activity(d, f"GitHub sync by @{update.effective_user.username or uid}: {result}")
    save_local(d)
    await msg.edit_text(result)

# ══════════════════════════════════════════════════════
#  ADMIN: /maintenance  (toggle via command)
# ══════════════════════════════════════════════════════
async def maintenance_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Maintenance mode is now {state}\n\n"
        + ("Users will see a maintenance message." if MAINTENANCE
           else "Users can use the bot normally.")
    )
...
async def button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Maintenance check for button presses (owner bypasses)
    ...
    if data == "home":
        ...
        MAINTENANCE = not MAINTENANCE
        state = "ON 🔧" if MAINTENANCE else "OFF ✅"
        await q.answer(f"Maintenance mode {state}", show_alert=True)
        await q.edit_message_text(
            build_admin_overview(d),
            reply_markup=kb_admin(),
        )
        ...
        if ok1 and ok2:
            ...
            result = "⚠️ Sync failed — token may be invalid. Check logs."
        else:
            result = "⚠️ Sync done with some errors on DB files."
        d2 = load()
...
async def on_startup(app: Application):
    ...
    ok = await gh_pull("data.json", DATA_FILE)
    if not ok:
        logger.warning("Startup: GitHub pull failed — using local data.json. Check your token.")
    d = load()
...
def main():
    ...
    app.add_handler(MessageHandler(
        ...
        ~filters.Regex(
            ...
            r"|revokekey|customname|syncgithub|addadmin|removeadmin|broadcast|maintenance)(?:@\S+)?(?:\s|$)"
        ),
        unknown_command,
    ))
