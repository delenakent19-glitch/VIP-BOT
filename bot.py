...
async def callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "home":
        ...
        bio = io.BytesIO("".join(chunk).encode())
        bio.name = f"{Path(fname).stem}.txt"
...
async def createkeys(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    key = "ZEIJIE-PREMIUM-" + "".join(random.choices(string.ascii_uppercase + string.digits, k=12))
...
async def redeem(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    d["redeemed"][uid] = {"key": key, "expires": None}
    save(d)
...
if __name__ == "__main__":
    main()    path = os.path.join(DB_FOLDER, file)
