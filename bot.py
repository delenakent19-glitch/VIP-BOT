    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if resp.status_code == 200:
                ...
                logger.error("GH push FAILED — token is invalid or expired!")
                return False
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            if resp.status_code == 401:
                logger.error("GH pull FAILED — token is invalid or expired!")
                return False
            if resp.status_code == 200:
...
def msg_key_generated(key: str, dur_label: str, devices: int) -> str:
    ...
    return (
        ...
        "- Auto-expiry enabled\n"
        "- Non-transferable\n\n"
        "Share this key with your user to grant premium access."
    )
...
def msg_file_caption(disp: str, sent: int, remaining: int) -> str:
    ...
    return (
        ...
        "Session      :  Verified\n\n"
        "Download immediately — file deletes in 5 min\n\n"
...
async def createkeys(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Tap to copy key:\n\n{key}")
...
async def bulkkeys(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    keys_block = "\n".join(keys)
    await update.message.reply_text(f"Tap to copy keys:\n\n{keys_block}")
...
async def button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "home":
        ...
        lines.append("\nUse /addadmin <id> or /removeadmin <id>")
        ...
        if not files:
            await q.edit_message_text(
                "No database files available right now.\n\n"
                f"Contact: {CONTACT_ADMIN}",
