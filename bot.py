...
# AUTO UI
# ═══════════════════════════════
async def send_main_ui(update, ctx):
    d = load()
...
async def callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "home":
        ...
        if not lines:
            await q.message.reply_text("❌ Database empty.")
            return
...
async def createkeys(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if duration == "lifetime":
        ...
        if "h" in duration:
            ...
            exp_dt = now + timedelta(minutes=num)
        else:
            exp_dt = now + timedelta(days=num)

        exp_text = exp_dt.strftime("%Y-%m-%d (%H:%M:%S)")
...
async def status_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not rd:
        await update.message.reply_text("🔒 No active key", reply_markup=kb_main(uid, d))
        return
...
async def redeem(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    exp_text = "♾️ Lifetime" if not exp else datetime.fromisoformat(exp).strftime("%Y-%m-%d %H:%M")
...
async def callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "home":
        await q.edit_message_text(LOGO, reply_markup=kb_main(uid, d))
        ...
        msg = await q.message.reply_text("🔍 Initializing 20%...")
        await asyncio.sleep(1)
        await msg.edit_text("⚙️ Processing 60%...")
        await asyncio.sleep(1)
        await msg.edit_text("📦 Finalizing 100%")
...
async def createkeys(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"🔑 Key Generated!\n{key}",
        reply_markup=kb_main(update.effective_user.id, d)
    )
